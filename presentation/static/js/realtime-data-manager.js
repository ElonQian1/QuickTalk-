...
class RealtimeDataManager {
    ...
    updateShopStatsDOM(shopId, statsData) {
        ...
        // 更新shop-stat显示
        const shopStatElements = document.querySelectorAll(
            `[data-shop-id="${shopId}"] .shop-stat`
        );
        
        shopStatElements.forEach(statContainer => {
            const label = statContainer.querySelector('.shop-stat-label');
            const value = statContainer.querySelector('.shop-stat-value');
            ...
            if (labelText === '未读') {
                value.textContent = statsData.unread_count || 0;
                this.debug('更新店铺未读统计:', statsData.unread_count);
            } else if (labelText === '对话') {
                value.textContent = statsData.conversation_count || 0;
                this.debug('更新店铺对话统计:', statsData.conversation_count);
            }
        });
        ...
        shopStatusElements.forEach(element => {
            ...
            element.textContent = hasConversations ? '有对话' : '等待中';
        });
    }
