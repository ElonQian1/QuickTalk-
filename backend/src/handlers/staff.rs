...
pub async fn list_staff(
    ...
    let items = services::staff::list_staff(&state.db, user_id, shop_id).await?;
    Ok(Json(items))
}
...
pub async fn add_staff(
    ...
    services::staff::add_staff(&state.db, user_id, shop_id, &payload.username).await?;
...
pub async fn remove_staff(
    ...
    services::staff::remove_staff(&state.db, user_id, shop_id, target_user_id).await?;
