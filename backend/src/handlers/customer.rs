...
pub async fn get_customers(
    ...
    for customer in customers {
        // 获取会话信息
        let session = state.db.get_session_by_shop_customer(shop_id, customer.id).await
            .unwrap_or(None);

        // 获取最后一条消息
        let last_message = if let Some(ref sess) = session {
            let messages = state.db.get_messages_by_session(sess.id, Some(1), Some(0)).await
                .unwrap_or_default();
        ...
        // 获取未读消息数
        let unread_count = state.db.get_unread_count(shop_id, customer.id).await
            .unwrap_or(0);
