...
class RealtimeDataManager {
    ...
    handleWebSocketMessage(message) {
        ...
        if (message.type === 'new_message') {
            ...
            // 修复：合并缓存并委托 DataSyncManager 更新 DOM
            const { shop_id, unread_count, conversation_count } = message.data || {};
            if (!shop_id || this.isTempId(shop_id)) return;
            const stats = {
                unread_count: typeof unread_count === 'number' ? unread_count : (this.shopStatsCache.get(shop_id)?.unread_count || 0),
                conversation_count: typeof conversation_count === 'number' ? conversation_count : (this.shopStatsCache.get(shop_id)?.conversation_count || 0)
            };
    ...
    updateConversationWithNewMessage(messageData) {
        ...
        // 更新店铺统计（本地叠加 + 委托 DataSyncManager 刷新/更新 DOM）
        this.updateShopStatsForNewMessage(shop_id);
        if (window.DataSyncManager && !this.isTempId(shop_id)) {
            // 优先使用统一逻辑刷新（含API回退与更鲁棒的选择器）
    ...
    updateShopStatsDOM(shopId, statsData) {
        // 将 DOM 更新委托给 DataSyncManager，以统一选择器与行为
        if (window.DataSyncManager && window.DataSyncManager.updateShopStatsDOM) {
            window.DataSyncManager.updateShopStatsDOM(shopId, statsData);
            return;
        }

        // 回退：简单更新（尽量不与 DataSyncManager 的逻辑冲突）
        // 已移除 .shop-stats 区域，直接更新状态和未读徽章
        const shopStatusElements = document.querySelectorAll(`[data-shop-id="${shopId}"] .shop-status`);
    ...
    async refreshShopStats(shopId) {
        ...
        // 优先委托 DataSyncManager 进行刷新（含鲁棒选择器与回退）
        if (window.DataSyncManager && window.DataSyncManager.forceRefreshShopStats) {
            try {
                ...
                this.debug('委托 DataSyncManager 刷新失败，回退本地逻辑:', e);
            }
        }
        try {
